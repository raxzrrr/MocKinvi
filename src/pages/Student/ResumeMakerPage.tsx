...
const ResumeMakerPage: React.FC = () => {
  ...
  const generateWithAI = async () => {
    ...
    try {
      ...
      // Build PDF directly from AI output without mutating form fields
      const doc = new jsPDF();
      let y = 20;
      ...
      if (result.summary) {
        ...
        y += 7;
        doc.setFont('helvetica', 'normal');
        const split = doc.splitTextToSize(result.summary, 170);
        doc.text(split, 20, y);
        y += split.length * 6 + 6;
      }
      ...
      if (Array.isArray(result.skills) && result.skills.length) {
        ...
        y += 7;
        doc.setFont('helvetica', 'normal');
        const skillLine = result.skills.join('  •  ');
        const split = doc.splitTextToSize(skillLine, 170);
        doc.text(split, 20, y);
        y += split.length * 6 + 6;
      }
      ...
      if (Array.isArray(result.certifications) && result.certifications.length) {
        ...
        y += 7;
        doc.setFont('helvetica', 'normal');
        const line = result.certifications.join('  •  ');
        const split = doc.splitTextToSize(line, 170);
        doc.text(split, 20, y);
        y += split.length * 6 + 6;
      }
      ...
      if (Array.isArray(result.experience) && result.experience.length) {
        ...
        result.experience.forEach((exp) => {
          ...
          if (Array.isArray(exp.bullets) && exp.bullets.length) {
            const bullets = exp.bullets.map(b => `• ${b}`);
      ...
      // Fallback: generate locally and export immediately
      const doc = new jsPDF();
      let y = 20;
      doc.setFont('helvetica', 'bold');
      doc.setFontSize(20);
      ...
      const yrsMatch = Math.max(experience.filter(e => e.start).length, 1);
      const core = skills.split(',').map(s => s.trim()).filter(Boolean).slice(0, 7);
      const awsFocus = core.find(s => s.toLowerCase().includes('aws')) ? 'AWS' : 'cloud';
      const coreText = core.length ? `Skilled in ${core.join(', ')}. ` : '';
      const summaryText = summary?.trim() || `Results-driven ${awsFocus} engineer with ${yrsMatch}+ years designing and operating highly-available systems on ${awsFocus}. ${coreText}Passionate about scalable architectures, cost optimization, and operational excellence.`;

      doc.setFont('helvetica', 'bold');
      doc.text('Professional Summary', 20, y);
      y += 7;
      doc.setFont('helvetica', 'normal');
      const splitSum = doc.splitTextToSize(summaryText, 170);
      doc.text(splitSum, 20, y);
      y += splitSum.length * 6 + 6;
      ...
      experience.forEach((exp) => {
        ...
        doc.text(`${exp.start || ''} - ${exp.end || ''}`, 20, y);
        y += 6;
        const bullets = inferAwsBullets(exp.description || `${exp.role} at ${exp.company}`);
        const split = doc.splitTextToSize(bullets.map(b => `• ${b}`).join('\n'), 170);
        doc.text(split, 20, y);
        y += split.length * 6 + 6;
      });
      ...
      <div className="p-6 space-y-6">
        <div className="flex items-center justify-between">
          ...
          <div className="flex gap-3">
            <Button variant="outline" onClick={generateWithAI} title="Polish content to AWS/ATS style">
