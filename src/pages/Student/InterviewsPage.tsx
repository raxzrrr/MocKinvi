...
import FacePresenceGuard, { FacePresenceGuardHandle } from '@/components/Interview/FacePresenceGuard';
...
const InterviewsPage: React.FC = () => {
  ...
  // Presence is observed but we no longer hard-pause input; we keep camera running continuously
  const presenceRef = useRef<FacePresenceGuardHandle | null>(null);
  ...
  const handleInterviewComplete = async (data: { 
    ...
    try {
      ...
      const scores = (bulkEvaluation?.evaluations || [])
        .map((e: any) => Number(e?.score) || 0)
        .filter((s: number) => s > 0);
      const avgScore = scores.length
        ? scores.reduce((a: number, b: number) => a + b, 0) / scores.length
        : 0;
      const isFallbackEvaluation = scores.length === 0;
      ...
      if (isFallbackEvaluation) {
        toast({
          ...
          description: "Your interview is saved. We'll improve the detailed evaluation and scoring shortly.",
        });
      } else {
        toast({
          title: "Interview Completed",
  ...
  const resetInterview = () => {
    ...
    setEvaluations([]);
    setSessionId('');
    try { presenceRef.current?.stop(); } catch {}
  };
      ...
      <DashboardLayout>
        ...
        <FacePresenceGuard
          ...
          onPresenceChange={(present) => {
            // Non-blocking: we no longer pause inputs to avoid flicker/lockups
          }}
        />
      </DashboardLayout>
    );
