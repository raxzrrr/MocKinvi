...
import { saveInterviewResultLocally } from '@/services/interviewService';
  ...
  useEffect(() => {
    ...
    const saveReport = async () => {
      try {
        ...
        await saveInterviewResultLocally(
          Math.round(averageScore * 10), // Convert to percentage
          interviewType,
          {
            technical: Math.round((evaluations.find(e => e.category === 'technical')?.score || 0) * 10),
            communication: Math.round((evaluations.find(e => e.category === 'communication')?.score || 0) * 10),
            problemSolving: Math.round((evaluations.find(e => e.category === 'problem_solving')?.score || 0) * 10),
            leadership: Math.round((evaluations.find(e => e.category === 'leadership')?.score || 0) * 10)
          }
        );
    ...
    saveReport();
  }, [questions, answers, evaluations, interviewType, averageScore, auth]);
        ...
        <TabsContent value="recommendations" className="space-y-4">
          <Card>
            ...
            <CardContent className="space-y-4">
              ...
              <div className="bg-purple-50 dark:bg-purple-500/10 p-4 rounded-lg border border-purple-200 dark:border-purple-400/20">
                <h4 className="font-medium text-purple-900 dark:text-purple-200 mb-2">Next Steps</h4>
                <div className="space-y-2 text-sm text-purple-800 dark:text-purple-200/90">
