  ...
  const handleDownloadCertificate = async () => {
    if (!results || !results.passed) {
      toast({
        ...
        description: 'You need to pass the assessment (70% or higher) to receive a certificate.',
        variant: 'destructive',
      });
      return;
    }
    ...
    try {
      // Generate certificate data with proper verification code
      ...
      // Import and use the certificate service
      const { downloadCertificate } = await import('@/services/certificateService');
      await downloadCertificate(certificateData);

      // Store certificate info in localStorage for the certificates page
      ...
      existingCerts.push(certificateInfo);
      localStorage.setItem('user_certificates', JSON.stringify(existingCerts));

      toast({
        title: 'Certificate Downloaded',
        description: 'Your certificate has been downloaded successfully! You can also view it in the Certificates page.',
      });
    } catch (error) {
