...
const VideoPlayer: React.FC<VideoPlayerProps> = ({ 
  ...
  // Prevent right-clicking on the player for download
  useEffect(() => {
    const handler = (e: MouseEvent) => {
      e.preventDefault();
      toast({ title: 'Download Restricted', description: 'Video downloading is not permitted.', variant: 'destructive' });
      return false;
    };
  ...
  useEffect(() => {
    ...
    const handleTimeUpdate = () => {
      if (video.duration > 0) {
        ...
        if (progressPercent >= 90 && progress < 90) {
          ...
          if (onAdvanceToNext) setTimeout(onAdvanceToNext, 1500);
        }
      }
    };
    ...
    const handleError = () => { setLoading(false); setError('Failed to load video. Please try again later.'); };
  ...
  const handleIframeError = () => { setLoading(false); setError('Failed to load video. Please try again later.'); };
  ...
  const handleMarkAsCompleted = () => {
    ...
    onProgress(100);
    onCompleted(moduleId);
    toast({ title: 'Module Completed', description: 'This module has been marked as completed.' });
    if (onAdvanceToNext) setTimeout(onAdvanceToNext, 1500);
  };
          ...
          <div className="relative w-full h-full">
            ...
            <div className="absolute bottom-0 left-0 right-0 bg-gradient-to-t from-black/70 to-transparent p-4">
              <div className="flex items-center space-x-4 text-white">
                <Button variant="ghost" size="sm" onClick={togglePlayPause} className="text-white hover:text-white hover:bg-white/20">
                  {isPlaying ? <Pause className="w-4 h-4" /> : <Play className="w-4 h-4" />}
                </Button>
                <div className="flex-1 h-2 bg-white/30 rounded-full cursor-pointer" onClick={handleSeek}>
                  <div className="h-full bg-brand-purple rounded-full transition-all duration-300" style={{ width: `${(currentTime / duration) * 100}%` }} />
                </div>
                <span className="text-sm">{formatTime(currentTime)} / {formatTime(duration)}</span>
                <Button variant="ghost" size="sm" onClick={toggleMute} className="text-white hover:text-white hover:bg-white/20">
      ...
      <div className="mt-4 space-y-4">
        <div>
          ...
          <div className="w-full h-2 bg-gray-200 rounded-full overflow-hidden">
            <div className="h-full bg-brand-purple rounded-full transition-all duration-300 ease-in-out" style={{ width: `${Math.min(progress, 100)}%` }} />
          </div>
        </div>
        <div className="flex justify-end">
          <Button onClick={handleMarkAsCompleted} className="bg-brand-purple hover:bg-brand-purple/90 font-medium" disabled={progress >= 100}>
