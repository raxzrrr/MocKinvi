...
import { adminService, AdminCourse, AdminCourseVideo, AdminCourseQuestion } from '@/services/adminService';
...
export const useCourseManagement = () => {
  ...
  const [videos, setVideos] = useState<Record<string, AdminCourseVideo[]>>({});
  const [questions, setQuestions] = useState<Record<string, AdminCourseQuestion[]>>({});
  ...
  const [editingVideo, setEditingVideo] = useState<AdminCourseVideo | null>(null);
  const [editingQuestion, setEditingQuestion] = useState<AdminCourseQuestion | null>(null);
  ...
  const fetchData = useCallback(async () => {
    try {
      ...
      // Use admin service to fetch all data
      const [coursesData, videosData, questionsData] = await Promise.all([
        adminService.fetchAllCourses(),
        adminService.fetchAllVideos(),
        adminService.fetchAllQuestions()
      ]);
  ...
  const handleAddCourse = useCallback(async (courseData: { name: string; description: string; order_index: number }) => {
    try {
      const newCourse = await adminService.addCourse({ ...courseData, is_active: true });
  ...
  const handleUpdateCourse = useCallback(async (course: AdminCourse) => {
    try {
      const updatedCourse = await adminService.updateCourse(course.id, course);
  ...
  const handleAddVideo = useCallback(async (videoData: {
    ...
    try {
      ...
      const newVideo = await adminService.addVideo({
        ...videoData,
  ...
  const handleUpdateVideo = useCallback(async (video: AdminCourseVideo) => {
    try {
      const updatedVideo = await adminService.updateVideo(video.id, video);
      setVideos(prev => ({
        ...prev,
        [video.course_id]: prev[video.course_id]?.map(v => v.id === video.id ? updatedVideo : v) || []
      }));
  ...
  const handleAddQuestion = useCallback(async (questionData: {
    ...
    try {
      ...
      const newQuestion = await adminService.addQuestion({
        ...questionData,
        course_id: selectedCourse.id
      });
  ...
  const handleUpdateQuestion = useCallback(async (question: AdminCourseQuestion) => {
    try {
      const updatedQuestion = await adminService.updateQuestion(question.id, question);
      setQuestions(prev => ({
        ...prev,
        [question.course_id]: prev[question.course_id]?.map(q => q.id === question.id ? updatedQuestion : q) || []
      }));
  ...
  const handleDeleteQuestion = useCallback(async (questionId: string, courseId: string) => {
    try {
      ...
      setQuestions(prev => ({
        ...prev,
        [courseId]: prev[courseId]?.filter(q => q.id !== questionId) || []
      }));
