...
const generateCertificate = async (supabase: any, params: {
  ...
  courseName: string;
  score: number;
  clerkUserId?: string; // Add clerkUserId parameter
}): Promise<{ success: boolean; certificateId?: string; message?: string }> => {
  const PASSING_SCORE = 70;
  
  console.log('generateCertificate called with params:', params);
  ...
  try {
    // FIRST: Update certificate management system if clerkUserId is provided
    if (params.clerkUserId) {
      try {
        console.log('Updating certificate management with params:', {
        ...
        const { data: certMgmtData, error: certMgmtError } = await supabase.rpc('update_course_certificate_management', {
          p_clerk_user_id: params.clerkUserId,
          p_course_id: params.courseId,
          p_course_name: params.courseName,
          p_course_complete: true,
        ...
        if (certMgmtError) {
          ...
          console.log('Certificate management updated successfully:', certMgmtData);
        }
      } catch (certMgmtError) {
        console.error('Error updating certificate management:', certMgmtError);
        // Continue with certificate generation even if management update fails
      }
    } else {
      console.log('No clerkUserId provided, skipping certificate management update');
    }
    ...
    let { data: defaultCertificate, error: certError } = await supabase
      .from('certificates')
      .select('*')
      .eq('is_active', true)
    ...
    if (certError || !defaultCertificate) {
      ...
      console.log('Created default certificate template:', defaultCertificate.id);
    }

    // Generate verification code
    const verificationCode = `CERT-${Date.now()}-${Math.random().toString(36).substring(2, 8).toUpperCase()}`;
    
    console.log('Creating user certificate with verification code:', verificationCode);
    
    // Use upsert to handle duplicates gracefully - save with both user_id and clerk_user_id for consistency
    const { data: newCert, error: saveError } = await supabase
      .from('user_certificates')
      .upsert({
        user_id: params.userId,
        clerk_user_id: params.clerkUserId, // Add clerk_user_id for consistency with certificate management
    ...
    console.log('Certificate generated successfully for user:', params.userId, 'course:', params.courseId, 'certId:', newCert.id);
  ...
  try {
    ...
    switch (action) {
      ...
      case 'evaluateAssessment':
        ...
        console.log('Received data:', { hasAnswers: !!answers, courseId: data?.courseId });
        
        if (!answers || !data?.courseId) {
          console.error('Missing required data:', { hasAnswers: !!answers, hasCourseId: !!data?.courseId });
          throw new Error('Answers and courseId are required for assessment evaluation');
        }
        ...
        // Check if user_learning record exists and update it properly
        const { data: existingLearning, error: checkError } = await supabase
          .from('user_learning')
          .select('*')
          .eq('user_id', supabaseUserId)
          .eq('course_id', data.courseId)
        ...
        if (checkError) {
          console.error('Error checking existing learning record:', checkError);
        }

        if (existingLearning) {
          // Update existing record - preserve existing progress data
          const updatePayload = {
            ...assessmentData,
          ...
          const { error: updateError } = await supabase
            .from('user_learning')
            .update(updatePayload)
            .eq('id', existingLearning.id); // Use the specific record ID
          ...
          console.log('Updated existing learning record:', existingLearning.id);
        } else {
          // Create new record
          const newRecord = {
            user_id: supabaseUserId,
          ...
          console.log('Created new learning record for assessment');
        }
        ...
        if (passed && data.courseName) {
          try {
            console.log('Attempting to generate certificate for passed assessment...');
            ...
            if (profileData?.full_name) {
              try {
                console.log('Calling generateCertificate with params:', {
                  userId: supabaseUserId,
                  name: profileData.full_name,
                ...
                if (certResult.success && certResult.certificateId) {
                  ...
                  certificateResult = { success: false, message: certResult.message || 'Certificate generation failed' };
                }
              } catch (certError) {
                console.error('Certificate generation failed:', certError);
